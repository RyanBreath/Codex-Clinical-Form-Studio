...
把臨床試驗計畫書依照可追溯、可審查、可測試、可重現的流程，協調轉換為核准的 YAML、有效的 JSON、預先編譯完成的 React HTML／JavaScript／CSS 網站，以及經 OpenAI Sites 驗證的發布成果。
...
- 強制 YAML review site 與 JSON runtime site 先完成 React production build，再進入 QA 與 Sites deployment。
- 在臨床或發布關鍵決策點要求人工確認。
...
3. 依序協調 MVP 流程：
   ...
   - YAML React Static Site Build
   - Clinical Field Review
   - YAML HTML QA
   - YAML-to-JSON
   - JSON React Static Site Build
   - JSON HTML QA
   - Human Release Approval
   - GitHub Versioning
   - OpenAI Sites Publish and Deployment Verification
...
- Static Frontend Build Gate Skill
- OpenAI Sites Deployment Gate Skill
...
- YAML／JSON React static bundle、asset manifest 與 checksum 狀態。
- OpenAI Sites project、site version 與 deployment reference。
...
7. YAML-to-HTML 與 JSON-to-HTML 必須使用版本化 React Render Engine 執行 production build，先產生 `index.html`、JavaScript、CSS、static assets 與 build manifest；不得把轉譯延後到 Sites backend 或 HTTP request 階段。
8. JSON 必須有效、可追溯，且 JSON React static bundle 完成後，才能進入 JSON HTML QA。
9. YAML HTML QA 與 JSON HTML QA 都必須針對「準備部署的同一份靜態 bundle」以可重現證據通過；不得以 dev server、SSR response 或口頭推定取代。
10. 只有在完整測試套件通過且取得明確 Human Release Approval 後，才能交由 SD 進行 GitHub versioning 與 OpenAI Sites publishing。
11. 同一個已核准來源 Artifact 與相同 engine／converter version，轉換結果及靜態 asset checksum 必須可重現；不一致時視為缺陷。
12. 缺陷必須按根因路由；修復後至少執行 focused retest，受共用 schema、converter、renderer 或 build configuration 影響時必須執行 regression QA。
13. 重複修復失敗、根因不明或 Gate 長期無法通過時，停止無限重試並升級給使用者或相應專家。
14. 每個 stage 完成、失敗、核准或 Artifact 版本改變後，都必須更新 project state。
...
| Git、branch、commit、Sites packaging、publish、deployment 或 platform 錯誤 | SD | SA 或 Orchestrator |
| React static build、asset manifest 或 request-time rendering dependency 錯誤 | SA | QA 與 SD |
...
- SA 已完成 YAML structural validation，並將 YAML review workspace 編譯成 React 靜態 HTML／JavaScript／CSS bundle。
- Clinic 或 human clinical reviewer 已核准 YAML fields。
- YAML HTML Playwright QA 已通過。
- SA 已產生有效且可追溯的 JSON runtime model。
- SA 已透過 Render Engine 將 JSON runtime form 編譯成 React 靜態 HTML／JavaScript／CSS bundle。
- JSON HTML Playwright QA 已通過。
- QA 已確認兩份網站不依賴 request-time backend YAML／JSON conversion 或 React SSR。
- Human release approval 已記錄。
- SD 已將核准 Artifact 納入 GitHub versioning。
- OpenAI Sites publishing 成功，且部署內容與通過 QA 的 build manifest／checksum 一致。
- Git／Sites version／deployment／published environment reference 已記錄。
...
- 不得將 YAML／JSON-to-HTML 轉譯交由 Sites backend、React SSR、RSC 或 request-time Worker 執行。
- 不得在 QA 後以不同設定重新建置並直接部署；若 bundle 改變必須重新測試。
