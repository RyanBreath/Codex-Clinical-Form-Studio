...
type Field = {
  ...
  label?: string;
  data_type?: string;
  required?: boolean | null | "unresolved";
  unit?: string | null;
...
function validationIssue(program: Program): string | null {
  if (!program.project_id) return "Confirmation requires project_id.";
  if (!program.selected_form?.candidate_id) return "Confirmation requires selected_form.candidate_id.";
  ...
  for (let index = 0; index < fields.length; index += 1) {
    ...
    if (field.calculation?.type === "bmi") {
      if (field.data_type !== "number" || field.calculation.decimal_places !== 2) {
        return `${name} requires a numeric BMI calculation rounded to 2 decimal places.`;
      }
      ...
      if (!height || height.data_type !== "number" || height.unit !== "cm") {
        return `${name} requires a numeric height source measured in cm.`;
      }
      if (!weight || weight.data_type !== "number" || weight.unit !== "kg") {
        return `${name} requires a numeric weight source measured in kg.`;
      }
    }
    ...
    if (coding.status === "matched") {
      if (
        ...
        !coding.source_url ||
        !coding.rationale ||
        coding.mapping_confidence_percent == null
      ) {
        return `${name} does not contain a complete evidence-backed CDISC mapping.`;
      }
    }
  }
  return null;
...
async function confirmYaml(request: Request) {
  ...
  if (/^\s*(?:%|---\s*$|\.\.\.\s*$)/m.test(body.yaml) || /(?:^|[\s[{,])(?:!<|![A-Za-z]|&[A-Za-z0-9_-]+|\*[A-Za-z0-9_-]+|<<\s*:)/m.test(body.yaml)) {
    return json({ error: "Unsafe YAML constructs are not supported." }, 400);
  }

  try {
    const program = parse(body.yaml, { uniqueKeys: true }) as Program;
